<?php 
      session_start();
if(isset($_SESSION['username']) && $_SESSION['role'] == "rec" || $_SESSION['role'] == "admin") {
     require 'config.php'; 
...
<div id="div_print" class="mdl-color-text--blue-grey-400">
    <?php
  $query_Events = "select * from guests where keygen='$keyche'";
$result = (mysqli_query($conn, $query_Events)) or die(mysqli_error($conn));
if(!$result){
echo mysqli_error();
} else{
...
<h1>Your PIN: <?php echo $keygen; ?></h1>
    <hr>
    <h6>You should use this PIN as a password for login into the app: <?php echo $name; ?>.hotelup.eu</h6>
    <p>For additional information, please call: <?php echo $tel; ?></p>
    <hr>
    <h4>Guest name: <?php echo $firstname . " " . $lastname; ?></h4>
    <h4>Room number: <?php echo $roomn; ?></h4>
    <hr>
    <table class="mdl-data-table mdl-js-data-table mdl-cell-10">
  <thead>
    <tr>
